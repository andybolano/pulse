...
export default function Planeacion() {
  ...
  return (
    <div className="min-h-screen bg-[#F7F9FF] pb-24 md:pt-[70px] md:pb-0">
      ...
      <div className="md:flex md:gap-6 md:items-start px-6 py-8 max-w-[1400px] mx-auto">

        {/* ── LEFT: Calendar ── */}
        <div className="flex-1 min-w-0">
          <h1 className="text-3xl font-extrabold mb-1" style={{ color: '#2F5BFF' }}>
            Planeación Semanal
          </h1>
          ...
          <div className="bg-white rounded-xl overflow-hidden border border-gray-200 shadow-sm">
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr>
                  <th className="w-16 py-3 px-3 text-white text-xs font-bold text-center" style={{ backgroundColor: '#1E3AAE' }}>
                    Hora
                  </th>
                  {diasHeader.map(d => (
                    <th
                      key={d.label}
                      className="py-3 px-2 text-white text-xs font-bold text-center"
              ...
              <tbody>
                {horas.map((hora) => (
                  <tr key={hora} className="border-t border-gray-200">
                    <td className="py-3 px-3 text-xs text-gray-400 font-medium whitespace-nowrap align-top">
                      {hora}
                    </td>
                          ...
                          {ev && (
                            <div
                              className={`rounded-lg px-2 py-1.5 text-xs font-semibold leading-tight w-full ${
                                ev.tipo === 'evento'
                                  ? 'text-white'
                                  : 'border border-dashed bg-[#F0EEFF] text-[#6A5CFF]'
                              }`}
          ...
          <div className="bg-white rounded-b-xl border border-t-0 border-gray-200 shadow-sm divide-y divide-gray-100">
            {sugerencias.map((s, i) => (
              <div key={i} className="flex items-center gap-3 px-4 py-4">
                ...
                <button
                  className="flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center text-white transition-all active:scale-95 cursor-pointer"
